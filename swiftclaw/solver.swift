...
func step(sol: Solution, dt: Double, num_ghost: Int, problem_data: [String: Double], solver_method: ([[Double]], [[Double]], [Double], [Double], [String: Double]) -> arrayGroup)
{
    
    var q_bc = [[Double]](count: sol.q.count, repeatedValue: [])
    for i in 0 ..< sol.q.count
    {
        q_bc[i] = [Double](count: 2*num_ghost+sol.q[0].count, repeatedValue: Double())
    ...
    var q_r = [[Double]](count: sol.q.count, repeatedValue: [Double](count: q_bc[0].count-1, repeatedValue: Double()))
    var q_l = [[Double]](count: sol.q.count, repeatedValue: [Double](count: q_bc[0].count-1, repeatedValue: Double()))
    ...
    for i in num_ghost ..< sol.gridCells+num_ghost
    {
        for mw in 0 ..< 1
        {
            cfl = [cfl, dt/sol.dx*Double(rp1.s[mw][i-num_ghost]), -dt/sol.dx*Double(rp1.s[mw][i-num_ghost])].maxElement()!
        }
    }
