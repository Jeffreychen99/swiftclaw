...
class Solution
{
    ...
    init(n: Int, i: Int, x: Double, x_space: Double, t: Double, method: (Double)->Double)
    {
        ...
        q = [[Double]](repeating: [], count: n)
        for index1 in 0 ..< num_eqn
        {
            var temp = [Double](repeating: Double(), count: i)
            for index2 in 0 ..< i
            {
    ...
    func writeToFile(_ frame: Int)
    {
        ...
        let locationQ = NSString(string: pathQ).expandingTildeInPath
        let locationT = NSString(string: pathT).expandingTildeInPath
        ...
        do {
            try qWritten.write(toFile: locationQ, atomically: true, encoding: String.Encoding.utf8)
        } catch _ {
        }
        ...
        tWritten += "1              num_dim\n \n"
        do {
            try tWritten.write(toFile: locationT, atomically: true, encoding: String.Encoding.utf8)
        } catch _ {
        }
